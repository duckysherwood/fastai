...
def get_sfs_idxs(sfs, last=True):
    ...
    if last:
        ...
        sfs_idxs = list(
            np.where(np.array(feature_szs[:-1]) != np.array(feature_szs[1:]))[
                0
            ]
        )
...
class UnetBlock(nn.Module):
    ...
    def __init__(self, up_in_c, x_in_c):
        ...
        self.upconv = nn.ConvTranspose2d(
            up_in_c, up_in_c // 2, 2, 2
        )  # H, W -> 2H, 2W
        self.conv1 = nn.Conv2d(
            x_in_c + up_in_c // 2, (x_in_c + up_in_c // 2) // 2, 3, 1, 1
        )
        self.conv2 = nn.Conv2d(
            (x_in_c + up_in_c // 2) // 2, (x_in_c + up_in_c // 2) // 2, 3, 1, 1
        )
    ...
    def forward(self, x):
        ...
        if not hasattr(self, "upmodel"):
            ...
            if imsize != self.sfs_szs[0][-2:]:
                ...
                self.extra_block = nn.ConvTranspose2d(
                    extra_in_c, extra_in_c, 2, 2
                )
