...
class ColumnarDataset(Dataset):
    ...
    def __init__(self, cats, conts, y, is_reg, is_multi):
        ...
        self.cats = np.stack(cats, 1).astype(np.int64) if cats else np.zeros(
            (n, 1)
        )
        self.conts = np.stack(conts, 1).astype(
            np.float32
        ) if conts else np.zeros(
            (n, 1)
        )
    ...
    def from_data_frames(
        cls, df_cat, df_cont, y=None, is_reg=True, is_multi=False
    ):
    ...
    @classmethod
    def from_data_frame(
        cls, df, cat_flds, y=None, is_reg=True, is_multi=False
    ):
        return cls.from_data_frames(
            df[cat_flds], df.drop(cat_flds, axis=1), y, is_reg, is_multi
        )
...
class ColumnarModelData(ModelData):
    ...
    def from_arrays(
        ...
        return cls(
            path,
            PassthruDataset(
                *(trn_xs.T), trn_y, is_reg=is_reg, is_multi=is_multi
            ),
            PassthruDataset(
                *(val_xs.T), val_y, is_reg=is_reg, is_multi=is_multi
            ),
    ...
    def from_data_frames(
        ...
        return cls(
            ...
            ColumnarDataset.from_data_frame(
                trn_df, cat_flds, trn_y, is_reg, is_multi
            ),
            ColumnarDataset.from_data_frame(
    ...
    def get_learner(
        ...
        **kwargs
        ...
        return StructuredLearner(
            self, StructuredModel(to_gpu(model)), opt_fn=optim.Adam, **kwargs
        )
...
class MixedInputModel(nn.Module):
    ...
    def forward(self, x_cat, x_cont):
        if self.n_emb != 0:
            x = [e(x_cat[:, i]) for i, e in enumerate(self.embatch_size)]
            x = torch.cat(x, 1)
            x = self.emb_drop(x)
        if self.n_cont != 0:
            x2 = self.bn(x_cont)
...
class StructuredModel(BasicModel):
    ...
    def get_layer_groups(self):
        m = self.model
        return [m.embatch_size, children(m.lins) + children(m.bns), m.outp]
...
class CollabFilterDataset(Dataset):
    ...
    def __init__(self, path, user_col, item_col, ratings):
        ...
        self.n = len(ratings)
        (
            self.users, self.user2idx, self.user_col, self.n_users
        ) = self.proc_col(
            user_col
        )
        (
            self.items, self.item2idx, self.item_col, self.n_items
        ) = self.proc_col(
            item_col
        )
    ...
    def proc_col(self, col):
        ...
        return (
            uniq, name2idx, np.array([name2idx[x] for x in col]), len(uniq)
        )
    ...
    def get_data(self, val_idxs, batch_size):
        val, trn = zip(*split_by_idx(val_idxs, *self.cols))
        return ColumnarModelData(
            self.path, PassthruDataset(*trn), PassthruDataset(*val), batch_size
        )
    ...
    def get_learner(self, n_factors, val_idxs, batch_size, **kwargs):
        return CollabFilterLearner(
            self.get_data(val_idxs, batch_size), self.get_model(n_factors), **kwargs
        )
