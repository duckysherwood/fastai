...
class AddPadding(CoordTransform):
    ...
    def do_transform(self, im, is_y):
        return cv2.copyMakeBorder(
            im, self.pad, self.pad, self.pad, self.pad, self.mode
        )
...
class NoCrop(CoordTransform):
    ...
    def do_transform(self, x, is_y):
        if is_y:
            return no_crop(x, self.sz_y, cv2.INTER_NEAREST)
        else:
...
class Scale(CoordTransform):
    ...
    def do_transform(self, x, is_y):
        if is_y:
            return scale_min(x, self.sz_y, cv2.INTER_NEAREST)
        else:
...
class RandomScale(CoordTransform):
    ...
    def do_transform(self, x, is_y):
        if is_y:
            return scale_min(x, self.store.new_sz_y, cv2.INTER_NEAREST)
        else:
