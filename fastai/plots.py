...
def plots(
    ims, figsize=(12, 6), rows=1, interp=False, titles=None, maintitle=None
):
...
def plots_from_files(
    imspaths, figsize=(10, 5), rows=1, titles=None, maintitle=None
):
...
class ImageModelResults():
    ...
    def most_uncertain_by_mask(self, mask, y):
        ...
        return idxs[
            np.argsort(np.abs(self.probs[idxs, y] - (1 / self.num_classes)))[
                :4
            ]
        ]
    ...
    def most_by_correct(self, y, is_correct):
        ...
        return self.most_by_mask(
            ((self.preds == self.ds.y) == is_correct) & (self.ds.y == y),
            y,
            mult,
        )
