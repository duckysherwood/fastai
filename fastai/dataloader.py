...
def get_tensor(batch, pin, half=False):
    if isinstance(batch, (np.ndarray, np.generic)):
        ...
        return {
            k: get_tensor(sample, pin, half) for k, sample in batch.items()
        }
    elif isinstance(batch, collections.Sequence):
        return [get_tensor(sample, pin, half) for sample in batch]
    raise TypeError(
        f"batch must contain numbers, dicts or lists; found {type(batch)}"
    )
...
class DataLoader(object):
        ...
        if batch_sampler is None:
            if sampler is None:
                sampler = RandomSampler(
                    dataset
                ) if shuffle else SequentialSampler(
                    dataset
                )
    ...
    def np_collate(self, batch):
        ...
        raise TypeError(
            (
                "batch must contain numbers, dicts or lists; found {}".format(
                    type(b)
                )
            )
        )
    ...
    def __iter__(self):
        if self.num_workers == 0:
            ...
            with ThreadPoolExecutor(max_workers=self.num_workers) as e:
                ...
                for c in chunk_iter(
                    iter(self.batch_sampler), self.num_workers * 10
                ):
