...
class LossRecorder(Callback):
    ...
    def plot_loss(self, n_skip=10, n_skip_end=5):
        ...
        plt.plot(
            self.iterations[n_skip:-n_skip_end],
...
class LearningRateFinder2(LearningRateFinder):
    ...
    def plot(self, n_skip=10, n_skip_end=5, smoothed=True):
        ...
        for i, m in enumerate(self.metrics):
            ...
            axs[i + 2].plot(
                self.lrs[n_skip:-n_skip_end], values[n_skip:-n_skip_end]
            )
        plt_val_l = smooth_curve(
            self.val_losses, 0.98
        ) if smoothed else self.val_losses
        axs[0].plot(
            self.lrs[n_skip:-n_skip_end], self.losses[n_skip:-n_skip_end]
        )
        axs[1].plot(
            self.lrs[n_skip:-n_skip_end], plt_val_l[n_skip:-n_skip_end]
        )
...
class CircularLR_beta(LR_Updater):
    ...
    def __init__(
        self, layer_opt, nb, div=10, pct=10, on_cycle_end=None, momentums=None
    ):
    ...
    def calc_lr(self, init_lrs):
        if self.cycle_iter > 2 * self.cycle_nb:
            pct = (self.cycle_iter - 2 * self.cycle_nb) / (
...
class WeightDecaySchedule(Callback):
    ...
    def on_batch_begin(self):
        ...
        if self.norm_wds:
            wdn = wdn / np.sqrt(
                self.batch_per_epoch * self.epoch_to_num_cycles[self.epoch]
            )
    ...
    def on_batch_end(self, loss):
        # Decay the weights
        for group, group_old, wds in zip(
            self.layer_opt.opt.param_groups,
            self.param_groups_old,
            self.new_wds,
        ):
...
class OptimScheduler(LossRecorder):
    ...
    def plot_lr(self, show_text=True, show_moms=True):
        ...
        for phase in self.phases:
            phase_limits.append(
                phase_limits[-1] + self.nb_batches * phase.epochs
            )
        if not in_ipynb():
            plt.switch_backend("agg")
