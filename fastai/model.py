...
class Stepper():
    ...
    def __init__(
        self, m, opt, crit, clip=0, reg_fn=None, fp16=False, loss_scale=1
    ):
...
def fit(
    ...
    **kwargs,
    ...
    get_ep_vals = kwargs.pop(
        "get_ep_vals"
    ) if "get_ep_vals" in kwargs else False
    metrics = metrics or []
    ...
    model_stepper = stepper(
        model, opt.opt if hasattr(opt, "opt") else opt, crit, **kwargs
    )
    ...
    for epoch in tnrange(tot_epochs, desc="Epoch"):
        ...
        for (*x, y) in t:
            ...
            stop = False
            los = debias_loss if not all_val else [
                debias_loss
            ] + validate_next(
                model_stepper, metrics, val_iter
            )
            for cb in callbacks:
        ...
        if not all_val:
            ...
            if swa_model is not None:
                if (
                    (epoch + 1) >= swa_start
                    and (
                        (epoch + 1 - swa_start) % swa_eval_freq == 0
...
def validate(stepper, dl, metrics):
    ...
    return [np.average(loss, 0, weights=batch_cnts)] + list(
...
def model_summary(m, input_size):
    ...
    if is_listy(input_size[0]):
        x = [
            to_gpu(Variable(torch.rand(3, *in_size))) for in_size in input_size
        ]
    else:
