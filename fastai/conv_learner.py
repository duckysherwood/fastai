...
model_features = {
    inception_4: 3072, dn121: 2048, dn161: 4416
}  # nasnetalarge: 4032*2}
...
class ConvLearner(Learner):
    ...
    def create_empty_bcolz(self, n, name):
        return bcolz.carray(
            np.zeros((0, n), np.float32), chunklen=1, mode="w", rootdir=name
        )
    ...
    def get_activations(self, force=False):
        ...
        if os.path.exists(names[0]) and not force:
            ...
            self.activations = [
                self.create_empty_bcolz(self.models.nf, n) for n in names
            ]
