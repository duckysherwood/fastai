...
class BOW_Dataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        if num_row_entries < self.max_len:
            # If short, pad
            indices = np.pad(
                indices, (self.max_len - num_row_entries, 0), mode="constant"
            )
            data = np.pad(
                data, (self.max_len - num_row_entries, 0), mode="constant"
            )
        else:
            # If long, truncate
...
class TextClassifierData(ModelData):
    ...
    def get_model(self, f, **kwargs):
        ...
        model = BasicModel(m)
        return BOW_Learner(
            self, model, metrics=[accuracy_thresh(0.5)], opt_fn=optim.Adam
        )
...
class LanguageModelLoader():
    ...
    def __init__(self, ds, bs, bptt, backwards=False):
        ...
        nums = fld.numericalize(
            [text], device=None if torch.cuda.is_available() else -1
        )
...
class ConcatTextDataset(torchtext.data.Dataset):
    ...
    def __init__(
        self, path, text_field, newline_eos=True, encoding="utf-8", **kwargs
    ):
...
class TextModel(BasicModel):
    ...
    def get_layer_groups(self):
        m = self.model[0]
        return [
            (m.encoder, m.dropouti), *zip(m.rnns, m.dropouths), (self.model[1])
        ]
