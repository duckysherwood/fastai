...
def train_lm(
    ...
    assert os.path.exists(PRE_LM_PATH), (
        "Error: %s does not exist." % PRE_LM_PATH
    )
    if lm_id != "":
        lm_id += "_"
    ...
    validation_downloader = LanguageModelLoader(val_lm, batch_size, bptt)
    md = LanguageModelData(PATH, 1, vs, training_downloader, validation_downloader, batch_size=batch_size, bptt=bptt)
    ...
    if not notrain:
        ...
        if early_stopping:
            callbacks.append(
                EarlyStopping(learner, lm_path, enc_path, patience=5)
            )
