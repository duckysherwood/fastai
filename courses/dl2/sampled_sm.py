...
def resample_vocab(itos, trn, val, sz):
    ...
    stoi2 = collections.defaultdict(
        lambda: 0, {v: k for k, v in enumerate(itos2)}
    )
    ...
    stoi3 = collections.defaultdict(
        lambda: 0, {v: k for k, v in enumerate(itos3)}
    )
...
class LinearDecoder(nn.Module):
    ...
    def __init__(
        self, n_out, nhid, dropout, tie_encoder=None, decode_train=True
    ):
        super().__init__()
...
def get_learner(drops, n_neg, sampled, md, em_sz, nh, nl, opt_fn, prs):
    ...
    crit = CrossEntDecoder(
        prs, m[1].decoder, n_neg=n_neg, sampled=sampled
    ).cuda()
